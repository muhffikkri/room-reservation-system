...
> Status merujuk **`dev`** (commit `b21af99` — setelah webp ter-merge). WebP conversion sudah dihapus. Refactor `refactor/reservation-availability` masih **Di Branch** dan belum masuk `dev`.
...
| 11 | CRUD fasilitas (tambah/edit/nonaktifkan) | ✔ Selesai | `Admin\FacilityController` + `FacilityRequest`; destroy = soft-disable via `nonaktifkan`/`aktifkan`; foto upload ke `storage/public/facilities` (simpan file asli); preview gambar di `app.js`. |
...
- **Media** (dev commit `b21af99`): foto fasilitas & laporan disimpan sebagai file asli (tanpa konversi WebP) via `intervention/image ^4.0`; tes menegaskan upload tersimpan `.jpg`/`.jpeg` < 500KB.
