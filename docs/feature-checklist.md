...
| Referensi commit | `5061c7a` (2026-09-09, Merge PR #23 — admin dashboard) |
...
| 3 | Daftar fasilitas publik + filter tipe/lokasi/kapasitas + grid ketersediaan **tanpa data pemohon** | ✗ Belum | Halaman publik `/fasilitas` belum dibangun (milestone Akbar). Data fasilitas + seeder sudah ada; `FacilityBookable` & status `aktif/perbaikan/nonaktif` siap dipakai. |
...
| 12 | Rekap okupansi & frekuensi kerusakan + ekspor CSV & PDF | ✗ Belum | `RecapService`, controller `Admin\RecapController`, view `admin/rekap/*` belum dibuat. Ditunda ke milestone v1. |
...
| BR-13 | Publik lihat fasilitas tanpa data pemohon | ✗ | Halaman publik belum |
...
## 3. Yang Sudah Ada (ringkasan artefak commit `5061c7a`)

- **30 route** (lihat `php artisan route:list`): auth custom, admin (dashboard, akun, verifikasi, fasilitas CRUD), petugas (dashboard, antrian reservasi).
- **90 tes Pest** lulus (305 assertions): auth, gate akun, verifikasi, akun admin/petugas, antrian reservasi petugas, CRUD fasilitas, dashboard admin, aturan slot/overlap (unit).
...
|---|---|---|
| Halaman publik fasilitas + jadwal grid | Akbar | `/fasilitas`, `/fasilitas/{id}/jadwal` tanpa data pemohon (BR-13) |
...
| Dashboard pengguna pengganti placeholder (`/dashboard`) | (jadwal) | Ringkasan reservasi & laporan milik sendiri |

---

*Terakhir diperbarui: 2026-09-09 · Komit referensi `5061c7a`.*